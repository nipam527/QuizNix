const express = require('express');
  ...
  const { updateAllowedUsers, getAllRooms } = require('../controllers/roomController');

  const verifyToken = require('../middleware/auth'); // assuming you have auth middleware


  const router = express.Router();
  router.put('/update-allowed-users/:code', verifyToken, updateAllowedUsers);
  ...
  router.get('/all', async (req, res) => {
    ...
    try {
      const rooms = await Room.find(); // assuming Mongoose is used
      res.json(rooms);
    } catch (error) {
